...
def run_unicode_point_distribution(
        unicode_code_points: str) -> pandas.DataFrame:
    unicode_point_distribution_dictionary = \
        __get_unicode_point_distribution_dictionary(
...
def __get_unicode_point_distribution_dictionary(
    ...
    unicode_point_distribution_dictionary = \
        {
            ord(unicode_code_point): unicode_code_points.count(unicode_code_point)
            for unicode_code_point
            in set(unicode_code_points)
        }
...
def __convert_unicode_point_distribution_dictionary_to_dataframe(
    ...
    unicode_point_distribution_dataframe.rename(
        columns={'index': 'unicode_point_codes'},
        inplace=True)
...
def __add_human_readable_columns(
    ...
    unicode_point_distribution_dataframe['unicode_point_names'] = \
        unicode_point_distribution_dataframe['unicode_point_names'].apply(
            lambda code: unicodedata.name(eval('u"\\u%04x"' % code), '-') if unicodedata.name(eval('u"\\u%04x"' % code),
                                                                                              '-') != '-' else '')
    ...
    unicode_point_distribution_dataframe['unicode_categories'] = \
        unicode_point_distribution_dataframe['unicode_categories'].apply(
            lambda code: unicodedata.category(eval('u"\\u%04x"' % code)) if unicodedata.name(eval('u"\\u%04x"' % code),
                                                                                             '-') != '-' else '')
